...
def get_data(filename):
	with open(filename, 'r') as csvfile:
		...
		for row in csvFileReader:
			...
			prices[0].append(float(row[1]))
			prices[1].append(float(row[2]))
	return
...
def predict_prices(dates, prices, x):
  ...
  svr_rbf.fit(dates, prices[0])
  svr_lin.fit(dates, prices[0])
  svr_poly.fit(dates, prices[0])

  # plt.scatter(dates, prices[0], color='black', label='Data')
...
predicted_price, coefficient, constant = predict_price(dates, prices, 22)  
print ("\nThe stock open price for 21st July is: $", str(predicted_price))
